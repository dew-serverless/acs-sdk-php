<?php

declare(strict_types=1);

namespace Dew\Acs\Adcp;

use Dew\Acs\AcsClient;

/**
 * @method \Dew\Acs\Result createHubCluster(array $arguments = [])
 * @method \Http\Promise\Promise createHubClusterAsync($arguments = [])
 * @method \Dew\Acs\Result describeHubClusters(array $arguments = [])
 * @method \Http\Promise\Promise describeHubClustersAsync($arguments = [])
 * @method \Dew\Acs\Result describeHubClusterDetails(array $arguments = [])
 * @method \Http\Promise\Promise describeHubClusterDetailsAsync($arguments = [])
 * @method \Dew\Acs\Result deleteHubCluster(array $arguments = [])
 * @method \Http\Promise\Promise deleteHubClusterAsync($arguments = [])
 * @method \Dew\Acs\Result attachClusterToHub(array $arguments = [])
 * @method \Http\Promise\Promise attachClusterToHubAsync($arguments = [])
 * @method \Dew\Acs\Result describeManagedClusters(array $arguments = [])
 * @method \Http\Promise\Promise describeManagedClustersAsync($arguments = [])
 * @method \Dew\Acs\Result detachClusterFromHub(array $arguments = [])
 * @method \Http\Promise\Promise detachClusterFromHubAsync($arguments = [])
 * @method \Dew\Acs\Result updateHubClusterFeature(array $arguments = [])
 * @method \Http\Promise\Promise updateHubClusterFeatureAsync($arguments = [])
 * @method \Dew\Acs\Result describeUserPermissions(array $arguments = [])
 * @method \Http\Promise\Promise describeUserPermissionsAsync($arguments = [])
 * @method \Dew\Acs\Result grantUserPermissions(array $arguments = []) {@deprecated 2022-01-01}
 * @method \Http\Promise\Promise grantUserPermissionsAsync($arguments = []) {@deprecated 2022-01-01}
 * @method \Dew\Acs\Result describeHubClusterKubeconfig(array $arguments = [])
 * @method \Http\Promise\Promise describeHubClusterKubeconfigAsync($arguments = [])
 * @method \Dew\Acs\Result grantUserPermission(array $arguments = [])
 * @method \Http\Promise\Promise grantUserPermissionAsync($arguments = [])
 * @method \Dew\Acs\Result updateUserPermission(array $arguments = [])
 * @method \Http\Promise\Promise updateUserPermissionAsync($arguments = [])
 * @method \Dew\Acs\Result deleteUserPermission(array $arguments = [])
 * @method \Http\Promise\Promise deleteUserPermissionAsync($arguments = [])
 * @method \Dew\Acs\Result describePolicies(array $arguments = [])
 * @method \Http\Promise\Promise describePoliciesAsync($arguments = [])
 * @method \Dew\Acs\Result describePolicyDetails(array $arguments = [])
 * @method \Http\Promise\Promise describePolicyDetailsAsync($arguments = [])
 * @method \Dew\Acs\Result deployPolicyInstance(array $arguments = [])
 * @method \Http\Promise\Promise deployPolicyInstanceAsync($arguments = [])
 * @method \Dew\Acs\Result describePolicyInstances(array $arguments = [])
 * @method \Http\Promise\Promise describePolicyInstancesAsync($arguments = [])
 * @method \Dew\Acs\Result describePolicyInstancesStatus(array $arguments = [])
 * @method \Http\Promise\Promise describePolicyInstancesStatusAsync($arguments = [])
 * @method \Dew\Acs\Result describePolicyGovernanceInCluster(array $arguments = [])
 * @method \Http\Promise\Promise describePolicyGovernanceInClusterAsync($arguments = [])
 * @method \Dew\Acs\Result deletePolicyInstance(array $arguments = [])
 * @method \Http\Promise\Promise deletePolicyInstanceAsync($arguments = [])
 * @method \Dew\Acs\Result describeHubClusterLogs(array $arguments = [])
 * @method \Http\Promise\Promise describeHubClusterLogsAsync($arguments = [])
 * @method \Dew\Acs\Result describeRegions(array $arguments = [])
 * @method \Http\Promise\Promise describeRegionsAsync($arguments = [])
 * @method \Dew\Acs\Result changeResourceGroup(array $arguments = [])
 * @method \Http\Promise\Promise changeResourceGroupAsync($arguments = [])
 */
final class AdcpClient extends AcsClient
{
    //
}
