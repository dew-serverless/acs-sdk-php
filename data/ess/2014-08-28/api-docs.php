  ...
  array (
    ...
    array (
      ...
      array (
        ...
        1 => 'DeleteScalingGroup',
        2 => 'ModifyScalingGroup',
        3 => 'EnableScalingGroup',
        4 => 'DisableScalingGroup',
        5 => 'SetGroupDeletionProtection',
        6 => 'AttachLoadBalancers',
        7 => 'DetachLoadBalancers',
        8 => 'DetachDBInstances',
        9 => 'AttachDBInstances',
        10 => 'AttachVServerGroups',
        11 => 'DetachVServerGroups',
        12 => 'DescribeScalingGroups',
        13 => 'DescribeScalingInstances',
        14 => 'DescribeScalingActivities',
        15 => 'DescribeScalingActivityDetail',
        16 => 'DescribeLimitation',
        17 => 'VerifyAuthentication',
        18 => 'SuspendProcesses',
        19 => 'ResumeProcesses',
        20 => 'VerifyUser',
      ),
    ...
    array (
      ...
      array (
        ...
        1 => 'DeleteScalingConfiguration',
        2 => 'ModifyScalingConfiguration',
        3 => 'DescribeScalingConfigurations',
        4 => 'DeactivateScalingConfiguration',
      ),
    ),
    2 => 
    array (
    ...
    array (
      ...
      array (
        ...
        1 => 'DeleteScheduledTask',
        2 => 'ModifyScheduledTask',
        3 => 'DescribeScheduledTasks',
      ),
    ),
    5 => 
    array (
      ...
      array (
        ...
        1 => 'DeleteLifecycleHook',
        2 => 'ModifyLifecycleHook',
        3 => 'DescribeLifecycleActions',
        4 => 'DescribeLifecycleHooks',
        5 => 'RecordLifecycleActionHeartbeat',
        6 => 'CompleteLifecycleAction',
      ),
    ),
    6 => 
    array (
      ...
      array (
        ...
        1 => 'DeleteNotificationConfiguration',
        2 => 'ModifyNotificationConfiguration',
        3 => 'DescribeNotificationConfigurations',
    ...
    array (
      ...
      'children' => 
      array (
        0 => 'SetInstancesProtection',
        1 => 'SetInstanceHealth',
        2 => 'EnterStandby',
        3 => 'ExitStandby',
        4 => 'RebalanceInstances',
      ),
    ),
    8 => 
    array (
    ...
    array (
      ...
      'children' => 
      array (
        0 => 'AttachAlbServerGroups',
        1 => 'CreateEciScalingConfiguration',
        2 => 'DeleteEciScalingConfiguration',
        3 => 'DescribeEciScalingConfigurations',
        4 => 'DetachAlbServerGroups',
        5 => 'ModifyEciScalingConfiguration',
        6 => 'ScaleWithAdjustment',
      ),
    ),
    12 => 
    array (
      ...
      array (
        ...
        1 => 'AttachServerGroups',
        2 => 'DetachServerGroups',
      ),
    ),
    13 => 
    array (
      ...
      'children' => 
      array (
        0 => 'ModifyInstanceAttribute',
        1 => 'DescribePatternTypes',
      ),
    ),
  ),
  'components' => 
...
- LOAD_BALANCER：根据负载均衡（暂不支持负载均衡CLB类型）健康检查结果判断实例的健康状态。

默认值：ECS。

>该参数与`HealthCheckTypes`参数作用相同，您可以根据实际情况进行二选一设置，即如果设置了`HealthCheckType`参数，则`HealthCheckTypes`参数将被忽略（非必填项）。',
            'type' => 'string',
            'required' => false,
            'example' => 'ECS',
          ),
...
>该参数与`HealthCheckType`参数作用相同，您可以根据实际情况进行二选一设置，即如果设置了`HealthCheckTypes`参数，则`HealthCheckType`参数将被忽略（非必填项）。',
            'type' => 'array',
            'items' => 
            array (
              'description' => '伸缩组的健康检查方式。取值范围：
...
|400|InvalidInstanceIds.PrivateIpNotFound|Can not find all private ips of instances in specific scaling group.|无法获取组内RDS实例的私网IP。',
    ),
    'AttachVServerGroups' => 
    array (
      'methods' => 
...
>该参数与`HealthCheckTypes`参数作用相同，查询结果只能存在二选一的情况，即`HealthCheckType`和`HealthCheckTypes`参数不能同时存在。',
                          'type' => 'string',
                          'example' => 'ECS',
                        ),
                        'LifecycleState' => 
...
>该参数与`HealthCheckType`参数作用相同，查询结果只能存在二选一的情况，即`HealthCheckType`和`HealthCheckTypes`参数不能同时存在。',
                              'type' => 'array',
                              'items' => 
                              array (
                                'description' => '伸缩组的健康检查方式。可能值：
...
例如`cn-hangzhou`、`cn-shanghai`。更多信息，请参见[地域和可用区](~~40654~~)。',
      ...
      'responseDemo' => '[{"type":"json","example":"{}","errorExample":""},{"type":"xml","example":"<VerifyUserResponse></VerifyUserResponse>","errorExample":""}]',
      'title' => '验证是否已开通弹性伸缩服务',
...
> InstanceType、InstanceTypes、InstanceTypeOverrides、Cpu和Memory 这四种方式不能同时指定，但InstanceType可以与InstancePatternInfos同时指定，此时，弹性伸缩会优先使用您指定的实例规格扩容，当指定的实例规格无库存时，再使用符合InstancePatternInfos 配置的实例规格。',
            'type' => 'string',
            'required' => false,
...
默认值：1。',
        ...
        array (
          ...
          array (
            ...
            'items' => 
            array (
              'description' => '多种实例规格参数。其中N的取值范围：1~10，即一个伸缩配置内最多可以设置10种实例规格。

N代表当前伸缩配置中实例规格的优先级，编号为1的实例规格优先级最高，实例规格优先级随着编号的增大依次降低。当无法根据优先级较高的实例规格创建出实例时，弹性伸缩服务会自动选择下一优先级的实例规格来创建实例。

>InstanceType、InstanceTypes、InstanceTypeOverrides、Cpu和Memory这四种方式不能同时指定，但InstanceTypes可以与InstancePatternInfos同时指定。此时，弹性伸缩优先使用您指定的实例规格扩容，当指定的实例规格无库存时，再使用符合InstancePatternInfos配置的实例规格。',
              'type' => 'string',
              'required' => false,
              'example' => 'ecs.g6.large',
            ),
          ...
          array (
            ...
            array (
              ...
              array (
                'PriceLimit' => 
                array (
                  'description' => '抢占式实例对应的出价，N的取值范围：1~10。SpotStrategy取值为SpotWithPriceLimit时生效。',
                  'type' => 'number',
                  'format' => 'float',
                  'required' => false,
                  'example' => '0.5',
                ...
                'InstanceType' => 
                array (
                  'description' => '抢占式实例的实例规格，N的取值范围：1~10。SpotStrategy取值为SpotWithPriceLimit时生效。',
                  'type' => 'string',
                  'required' => false,
                  'example' => 'ecs.g6.large',
                ),
...
- cloud_auto：ESSD AutoPL云盘。',
                        ...
                        array (
                          'description' => '公网出带宽最大值，单位为Mbps（Mega bit per second）。可能值：
...
- ARM。',
                                  ...
                                  'MinimumGpuAmount' => 
                                  array (
                                    'description' => '实例期望的最小GPU数目。可能值：≥1的正整数。',
                                    'type' => 'integer',
                                    'format' => 'int32',
                                    'example' => '2',
                                  ),
                                  'MaximumGpuAmount' => 
                                  array (
                                    'description' => '实例期望的最大GPU数目。可能值：≥1的正整数。',
                                    'type' => 'integer',
                                    'format' => 'int32',
                                    'example' => '2',
                                  ),
        ...
        array (
          ...
          array (
            ...
            array (
              'ScalingActivityId' => 
              array (
                'description' => '移出ALB服务器组，并移出该ALB服务器组内的ECS实例时，伸缩活动的ID。仅当`ForceDetach`取值为`true`时，返回该参数。',
                'type' => 'string',
                'example' => 'asa-2ze6wxj8vsohn6j9****',
              ),
              'RequestId' => 
